...
export async function GET(req: NextRequest) {
    try {
        ...
        // Special case for native token (ETH on Arbitrum)
        if (address.toLowerCase() === "0x0000000000000000000000000000000000000000") {
            return NextResponse.json({
                ...
                token: {
                    ...
                    chainId: 42161,
                    isNative: true,
                    logoUrl: getExplorerTokenLogoUrl('arb', address)
                }
            });
        }

        // Get Arbitrum RPC URL from network map
        const rpcUrl = NETWORK_MAP.arb.rpcUrls.http[0];
        ...
        // Make parallel requests to the Arbitrum node
        ...
        if (nameData.error || symbolData.error || decimalsData.error) {
            ...
            if (isContractError || isNotFoundError) {
                ...
                return NextResponse.json({ 
                    success: false, 
                    error: "No valid token found at this address on Arbitrum network"
                }, { status: 404 });
            } else {
        ...
        console.error("Error validating Arbitrum token:", error);
        return NextResponse.json({
            success: false,
