...
async function getTokenLogo(network: string, address: string): Promise<string> {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_APP_URL || ''}/api/tokens-logo?network=${network}&address=${address}`,
...
export async function GET(req: NextRequest) {
    try {
        ...
        // Special case for native token (AVAX)
        if (address.toLowerCase() === "0x0000000000000000000000000000000000000000") {
            // Get logo from our token logo API
            const logoUrl = await getTokenLogo('avax', address);
        ...
        // Get Avalanche RPC URL from network map
        const rpcUrl = NETWORK_MAP.avax.rpcUrls.http[0];
        ...
        // Make parallel requests to the Avalanche node
        ...
        if (nameData.error || symbolData.error || decimalsData.error) {
            ...
            if (isContractError || isNotFoundError) {
                ...
                return NextResponse.json({ 
                    success: false, 
                    error: "No valid token found at this address on Avalanche network" 
                }, { status: 404 });
            } else {
        ...
        console.error("Error validating Avalanche token:", error);
        return NextResponse.json({
            success: false,
