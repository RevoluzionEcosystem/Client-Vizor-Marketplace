...
export async function GET(req: NextRequest) {
    try {
        ...
        // Special case for native token (ETH on Base)
        if (address.toLowerCase() === "0x0000000000000000000000000000000000000000") {
            return NextResponse.json({
                ...
                token: {
                    ...
                    chainId: 8453,
                    isNative: true,
                    logoUrl: getExplorerTokenLogoUrl('base', address)
                }
            });
        }

        // Get Base RPC URL from network map
        const rpcUrl = NETWORK_MAP.base.rpcUrls.http[0];
        ...
        // Make parallel requests to the Base node
        ...
        if (nameData.error || symbolData.error || decimalsData.error) {
            ...
            if (isContractError || isNotFoundError) {
                ...
                return NextResponse.json({ 
                    success: false, 
                    error: "No valid token found at this address on Base network" 
                }, { status: 404 });
            } else {
        ...
        console.error("Error validating Base token:", error);
        return NextResponse.json({
            success: false,
