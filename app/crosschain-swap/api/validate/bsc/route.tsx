...
export async function GET(req: NextRequest) {
    try {
        ...
        // Special case for native token (BNB)
        if (address.toLowerCase() === "0x0000000000000000000000000000000000000000") {
            return NextResponse.json({
                ...
                token: {
                    ...
                    chainId: 56,
                    isNative: true,
                    logoUrl: getExplorerTokenLogoUrl('bsc', address)
                }
            });
        }

        // Get BSC RPC URL - use fallback if NETWORK_MAP is unavailable
        let rpcUrl;
        try {
            rpcUrl = NETWORK_MAP?.bsc?.rpcUrls?.http?.[0] || FALLBACK_RPC_URLS.bsc;
        } catch (error) {
            console.log("Error accessing NETWORK_MAP, using fallback:", error);
            rpcUrl = FALLBACK_RPC_URLS.bsc;
        }
        ...
        // Make parallel requests to the BSC node
        ...
        console.log("API responses:", { nameData, symbolData, decimalsData });
        ...
        if (nameData.error || symbolData.error || decimalsData.error) {
            ...
            if (isContractError || isNotFoundError) {
                ...
                return NextResponse.json({ 
                    success: false, 
                    error: "No valid token found at this address on BSC network" 
                }, { status: 404 });
            } else {
        ...
        console.error("Error validating BSC token:", error);
        return NextResponse.json({
            success: false,
