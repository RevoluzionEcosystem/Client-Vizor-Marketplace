...
const walletAddressValidator = z.string().refine(
    (address) => /^0x[a-fA-F0-9]{40}$/.test(address) || /^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(address),
    {
...
export const getCloudTransactionHistory = cache(async (
    ...
    if (typeof window === "undefined" || !address) {
        ...
        const response = await fetch(`/api/user-history/crosschain/crosschain-swap?walletAddress=${address}&limit=${limit}`);
...
export async function saveCloudTransaction(
    ...
    if (typeof window === "undefined" || !address) {
        ...
        const response = await fetch('/api/user-history/crosschain/crosschain-swap', {
            method: 'POST',
            headers: {
...
export async function getTransactionHistory(
    ...
    try {
        ...
        const cloudHistory = await getCloudTransactionHistory(address, limit);
